...
For this, *Nexus* uses a mixture of [[github:minad/corfu][corfu]] with [[github:minad/cape][cape]]. This handles the completion backend that gets used throughout.
...
Cape allows us to extend the completion backends by using the provided functions to extend the capf list.

#+begin_src emacs-lisp
(use-package cape
  :ensure t
  :init
  ;; Add file completions to prog modes
  (add-hook 'prog-mode-hook
            (lambda ()
              (add-hook 'completion-at-point-functions #'cape-file -10 t)))

  ;; Add dabbrev globally
  ...
  ;; Make sure lsp suggesstions are noninterruptible
...
Finally we just need to provide the file
...
** Search
:PROPERTIES:
:header-args:emacs-lisp: :tangle "lisp/packages/search-module.el" :eval no
:END:

Searching and filtering in *Nexus* is aided through the use of [[github:minad/vertico][vertico]] and [[github:minat/consult][consult]], with vertico acting as the frontend and consult as the backend.

*** Vertico

Vertico creates the UI needed for any kind of builtin or external searching and filtering.
